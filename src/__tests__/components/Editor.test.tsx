...
describe('Editor', () => {
  it('渲染标题输入', () => {
    render(
      <Editor note={mockNote} onUpdate={vi.fn()} onSave={vi.fn()} onDelete={vi.fn()} />
    )
  ...
  it('渲染保存按钮', () => {
    ...
    expect(screen.getByText('保存笔记')).toBeInTheDocument()
  })
  ...
  it('切换笔记时同步内容', () => {
    ...
    rerender(
      <Editor note={note2} onUpdate={vi.fn()} onSave={vi.fn()} onDelete={vi.fn()} />
    )
